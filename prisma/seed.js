...
// Simple hash for seeding (in production, use bcrypt from Part 2)
// For now, we store plain text passwords that will be hashed in Part 2
...
async function main() {
    ...
    const admin = await prisma.user.upsert({
        ...
        create: {
            ...
            passwordHash: SEED_PASSWORD, // Will be bcrypt-hashed in Part 2
            role: 'ADMIN',
            name: 'System Admin',
    ...
    // ==========================================
    // 2. PARENT USERS (matching existing mock data)
    // ==========================================
    const parents = [
        {
            email: 'akarim.dxb@gmail.com',
            name: 'Abdul Karim',
    ...
    for (const p of parents) {
        ...
        console.log('✅ Parent created:', user.name, `(${user.city}, ${user.country})`);
    }

    // ==========================================
    // 3. CHILDREN (matching existing mock data)
    // ==========================================
    const childrenData = [
        ...
        { parentIdx: 4, name: 'Ayaan', age: 12, phone: '017XX XXXXXX' }, // Sobuj's child
    ];
    ...
    for (const c of childrenData) {
        const child = await prisma.child.create({
            data: {
                ...
                age: c.age,
                phone: c.phone,
                parentId: createdParents[c.parentIdx].id,
            },
        });
        createdChildren.push(child);
        console.log(`  👶 Child: ${child.name} (${child.age}y) → Parent: ${createdParents[c.parentIdx].name}`);
    }

    // ==========================================
    // 4. DEVICES (one per child)
    // ==========================================
    const devicesData = [
    ...
    for (const d of devicesData) {
        await prisma.device.create({
            data: {
                ...
                childId: createdChildren[d.childIdx].id,
            },
        });
    }
    ...
    const subsData = [
        ...
        { parentIdx: 2, plan: 'TRIAL', cycle: 'MONTHLY', amount: 0, months: 0, method: null },   // Expired trial
    ...
    for (const s of subsData) {
        ...
        const end = new Date();
        if (s.plan === 'TRIAL') {
            end.setDate(end.getDate() - 1); // Already expired
        } else {
        ...
        await prisma.subscription.create({
            data: {
                plan: s.plan,
                status: s.plan === 'TRIAL' ? 'EXPIRED' : 'ACTIVE',
                billingCycle: s.cycle,
                amount: s.amount,
                startDate: start,
                endDate: end,
                paymentMethod: s.method,
                userId: createdParents[s.parentIdx].id,
            },
        });
    }
    ...
    // ==========================================
    // 6. TRANSACTIONS (matching BillingTab mock data)
    // ==========================================
    const trxData = [
    ...
    for (const tx of trxData) {
        ...
        await prisma.transaction.create({
            data: {
                ...
                status: tx.status,
                createdAt,
                userId: createdParents[tx.parentIdx].id,
            },
        });
    }
    ...
    // ==========================================
    // 7. ALERTS (matching dashboard mock data)
    // ==========================================
    const alertsData = [
    ...
    for (const a of alertsData) {
        ...
        await prisma.alert.create({
            data: {
                ...
                severity: a.severity,
                createdAt,
                childId: createdChildren[a.childIdx].id,
            },
        });
    }
    ...
    for (const g of geoData) {
        await prisma.geofenceZone.create({
            data: {
                ...
                longitude: g.lng,
                radius: g.radius,
                childId: createdChildren[g.childIdx].id,
            },
        });
    }
    ...
    for (const app of appData) {
        await prisma.appControl.create({
            data: {
                ...
                usageToday: app.used,
                iconColor: app.color,
                childId: createdChildren[app.childIdx].id,
            },
        });
    }
    ...
    for (const p of prayerData) {
        await prisma.prayerTimeLock.create({
            data: {
                ...
                endTime: p.end,
                isActive: p.active,
                childId: createdChildren[p.childIdx].id,
            },
        });
    }
    ...
    // ==========================================
    // 11. CONTENT FILTERS (Global admin rules)
    // ==========================================
    const filterData = [
    ...
    for (const tk of ticketData) {
        ...
        await prisma.supportTicket.create({
            data: {
                ...
                createdAt,
                userId: createdParents[tk.parentIdx].id,
            },
        });
    }
